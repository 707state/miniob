...
#include "alter_table_modify_executor.h"
RC CommandExecutor::execute(SQLStageEvent *sql_event)
{
  Stmt *stmt = sql_event->stmt();

  switch (stmt->type()) {
    case StmtType::ALTER_INDEX_ADD: {
      AlterTableAddExecuter executor;
      return executor.execute(sql_event);
    } break;
    case StmtType::ALTER_INDEX_DROP: {
      AlterTableDropExecuter executor;
      return executor.execute(sql_event);
    } break;
    case StmtType::ALTER_INDEX_MODIFY: {
      AlterTableModifyExecutor executor;
      return executor.execute(sql_event);
    } break;
    case StmtType::ALTER_COLUMN_ADD: {
      AlterColumnAddExecutor executor;
      return executor.execute(sql_event);
    } break;
    case StmtType::ALTER_COLUMN_DROP: {
      AlterColumnDropExecutor executor;
      return executor.execute(sql_event);
    } break;
    case StmtType::ALTER_COLUMN_MODIFY: {
      AlterColumnModifyExecutor executor;
      return executor.execute(sql_event);
    } break;
